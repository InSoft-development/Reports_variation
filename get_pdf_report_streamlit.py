...
def StringGuy(text):
    return f'<font name="FS">{text}</font>'


# форматирование текста легенды
def StringGuy_legend(text, color):
    return f'<font name="FS">{text[:text.index(":")]}' \
           f'</font><font name="FS" color={color}>{text[text.index(":"):]}</font>'
...
def get_common_report(fig_home, df_common, data_df, merged_interval_list, interval_list, tab_name_list,
    ...
    pdfmetrics.registerFont(TTFont('FS', 'FreeSerif.ttf', 'UTF-8'))
